...
# speedtest2mqtt@<name> services there. Same script as in lgtv2mqtt / lgsb2mqtt.
#
# Usage:
...
#
# Optional env vars:
#   REMOTE_DIR   (default: /usr/local/lib/node_modules/speedtest2mqtt)
#   REMOTE_TMP   (default: /tmp)
#   SERVICE      systemd unit(s) to restart after deploying; default: every active
#                speedtest2mqtt@<name> instance plus a plain speedtest2mqtt unit if present
...
if [[ -n "$SERVICE" ]]; then
  ...
  echo "No $PKG service found to restart (install one with: sudo $PKG --install -n speedtest -u mqtt://broker)"
fi
